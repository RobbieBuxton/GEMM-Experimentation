...
int compare_stencil(int steps, int size_scale, int size_start, int iterations_scale, int iterations_start, float gamma, float a_n1, float a_1, float b_n1, float b_1)
{
	...
	for (int i = 0; i <= steps; i++)
	{
		...
		int size = i * size_scale + size_start;
		int iterations = i * iterations_scale + iterations_start;
		printf("\nIterations: %d Size: %d\n", iterations, size);
		...
		printf("Max Percent Error: %f Average Percent Error: %f\n", max_error, cum_error / ((size - 2) * (size - 2)));

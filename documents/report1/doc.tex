...
I believe this algorithm has complexity $O(M)$ as it is bound by the time required to compute 4 matrix multiplications. $M <= n^3$ depending 
on how sparse the innersum becomes which leads to the strange but correct behaviour where larger iterations become quicker to compute \\ \\
...
So I believe the diagonalisation has a complexity $O(n^2)$ \\ \\
...
&= (\transformedSourceGrid_{i,j})  ({\horizontalEigens_{j,j}})^{n} (1 + \frac{\verticalEigens_{i,i}}{\horizontalEigens_{j,j}})^{n} &&\\
